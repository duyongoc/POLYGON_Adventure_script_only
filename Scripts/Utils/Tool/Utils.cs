...
public class Utils : MonoBehaviour
{
    ...
    public static float ConvertMoveSpeedToNavMeshValue(float value)
    {
        if (value == 0)
            return 2.0f;

        // exp: =>> 350 / 100 =  3.5  =>> 100 is CONST
        return value / CONST.FACTOR_MOVE_SPEED;
    }


    public static float ConvertAttackSpeedToTimeValue(float value)
    {
        if (value == 0)
            return 2.0f;

        // exp: =>> 1 / (200 / 300) = 1.5  =>> 300 is CONST
        return 1 / (value / CONST.FACTOR_ATTACK_SPEED);
    }
