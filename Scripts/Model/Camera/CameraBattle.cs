...
public class CameraBattle : MonoBehaviour
{
    ...
    private void Update()
    {
        if (BattleController.Instance.IsGameFinished)
            return;
    ...
    private void OnUpdate()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                actorMain.SetMove(hit.point, true);

                var indexPosition = 0;
                var positionList = GetPositionListAround(hit.point, 1.5f, actorList.Count);
                foreach (var player in actorList)
                {
                    if (player.name.Equals(actorMain.name))
                        continue;

                    player.SetMove(positionList[indexPosition], false);
                    indexPosition = (indexPosition + 1) % positionList.Count;
                }
            }
        }
    ...
    public void MoveToMainCharacter()
    {
        var indexPosition = 0;
        var positionList = GetPositionListAround(actorMain.transform.position, 1.5f, actorList.Count);
        ...
        foreach (var player in actorList)
        {
            ...
            player.SetMove(positionList[indexPosition], true);
            indexPosition = (indexPosition + 1) % positionList.Count;
        }
    }
